...
def lgbm_mv(ti,oos_length,row_counter,debug_models,variable_list,n_splits,validation_window):
  import numpy as np
...
#     print('y_pred:\n',y_pred)
  ...
  for train_index, test_index in tscv.split(select_X):
   ...
   # map_error.append(wmape(true_values,y_pred))
    forecast= pd.concat([pd.Series(ti.index[len(cv_train):len(cv_train)+len(cv_test)]),pd.Series(model1.predict(cv_test)),pd.Series(['lgbm_mv']*len(cv_test))],axis=1)
 ...
 # model=model1.best_estimator_.fit(select_X,ti)
  ...
  forecast2=pd.concat([pd.Series(oos.index),pd.Series(model1.predict(select_X_oos)),pd.Series(['lgbm_mv']*h)],axis=1)
